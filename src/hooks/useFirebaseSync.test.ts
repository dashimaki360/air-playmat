...
describe('computeFirebaseUpdates', () => {
    ...
    it('カード移動で変更されたカードとデッキ配列のみを含む', () => {
        const state = createInitialState();
        const handCard = Object.values(state.c).find(c => c.l === 'p1-hand')!;
        ...
        expect(updates[`rooms/${roomId}/state/c/${handCard.id}`]).toBeDefined();
        expect(updates[`rooms/${roomId}/state/m/a`]).toBeDefined();

        // p2側のカードは含まれないべき
        const p2CardKeys = Object.keys(updates).filter(k => k.includes('/c/p2-'));
        expect(p2CardKeys.length).toBe(0);
    });
    ...
    it('drawCardでデッキ配列とカードの差分のみを含む', () => {
        ...
        // デッキ配列の変更
        expect(updates[`rooms/${roomId}/state/d/p1`]).toBeDefined();
        // lastAction
        expect(updates[`rooms/${roomId}/state/m/a`]).toBeDefined();
    });
    ...
    it('attachCardで変更されたカードのみを含む', () => {
        const state = createInitialState();
        const [first, second] = Object.values(state.c).filter(c => c.l === 'p1-hand');
        ...
        // attach されたカードの変更
        expect(updates[`rooms/${roomId}/state/c/${second.id}`]).toBeDefined();
        // lastAction
        expect(updates[`rooms/${roomId}/state/m/a`]).toBeDefined();
    });
    ...
    it('updateCardStatusでダメージ変更のみ', () => {
        const state = createInitialState();
        const card = Object.values(state.c).find(c => c.l === 'p1-hand')!;
        const next = applyUpdateCardStatus(state, card.id, c => ({ ...c, d: 60 }));

        const updates = computeFirebaseUpdates(roomId, state, next);

        expect(updates[`rooms/${roomId}/state/c/${card.id}`]).toBeDefined();
